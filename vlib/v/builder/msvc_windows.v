...
pub fn (mut v Builder) cc_msvc() {
	...
	app_dir_out_name_c := os.real_path(v.pref.out_name_c).all_before_last('.')
	mut a := []string{}
	...
	// Keep all MSVC sidecar paths relative to the final absolute output name.
	// In particular, the .def generated by cgen must be the same file passed to
	// the linker, even when -o contains nested directories or forward slashes.
	app_dir_out_name := if v.pref.out_name.ends_with('.dll') || v.pref.out_name.ends_with('.exe') {
		v.pref.out_name[0..v.pref.out_name.len - 4]
	} else {
		v.pref.out_name
	}
	if !v.pref.is_debug && !v.ccoptions.debug_mode {
